    ...
    for m in cand_models:
        ...
        while retries <= max_retries_per_model:
            try:
                ...
                err_str = str(e).lower()
                last_error = str(e)
                logger.warning(f"Groq error on model {m}: {e}")
                
                # invalid key -> fail fast
                if "401" in err_str or "invalid" in err_str:
                    return {"error": "INVALID_KEY"}
                
                # Rate limit or connection error -> backoff & retry
                if "429" in err_str or "connection" in err_str or "timeout" in err_str:
                    retries += 1
                    sleep_time = 3 * retries  # increased backoff
                    logger.info(f"Error detected. Retry {retries}/{max_retries_per_model} on {m} in {sleep_time}s.")
